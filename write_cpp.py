#!/usr/bin/env python3
"""Write the fixed PowerShellScanner.cpp"""
import os
...
// ── AMSI bypass strings (all lowercase for direct comparison) ────────────────
...
// ── Download cradle patterns (deduplicated from SUSPICIOUS_CMDLETS) ──────────
// Only patterns NOT already in SUSPICIOUS_CMDLETS to avoid double-scoring.
...
public:
    ...
    Impl() {
        ...
        try {
            ...
            SS_LOG_ERROR(LOG_CAT, L"Failed to init default blacklist: %hs", ex.what());
        }
    ...
    [[nodiscard]] ScanResult scanFile(
        ...
        try {
            ...
            result = analyzeContent(content, path.filename().string(), pid);
            result.filePath  = path.string();
            result.sha256    = Utils::HashUtils::ToHexLower(hashBytes.data(), hashBytes.size());
            result.processId = pid;
            result.mode      = ExecutionMode::FILE_ON_DISK;
    ...
    [[nodiscard]] ScanResult scanMemory(
        ...
        try {
            ...
            result = analyzeContent(contentStr, std::string(sourceDescription), pid);
            result.processId = pid;
            result.mode      = ExecutionMode::MEMORY_ONLY;
            result.scanTime  = std::chrono::system_clock::now();
    ...
    [[nodiscard]] ScanResult scanCommandLine(
        ...
        uint32_t pid,
        bool suppressSideEffects = false
    ) noexcept {
        const auto startTime = std::chrono::high_resolution_clock::now();
        ScanResult result;
        result.scanTime  = std::chrono::system_clock::now();
        ...
        try {
            if (commandLine.empty()) {
                result.status = ScanStatus::CLEAN;
                return finalize(result, startTime, suppressSideEffects);
            }
            ...
            if (extractEncodedCommand(std::string(commandLine), encodedPayload)) {
                ...
                if (!decoded.empty()) {
                    auto inner = analyzeContent(decoded, "DecodedCommand", pid);
            ...
            SS_LOG_FATAL(LOG_CAT, L"Unknown exception in scanCommandLine");
            result.status      = ScanStatus::ERROR_INTERNAL;
            result.description = "Unknown internal error";
        }
    ...
    [[nodiscard]] bool healthCheck() noexcept {
        ...
        // Known-malicious must trigger — use analyzeContent directly (no IPC side effects)
        ...
        // V2 detection — suppressSideEffects to avoid kernel IPC during health check
        const std::string testV2 = "powershell.exe -version 2 -command test";
        auto r3 = scanCommandLine(testV2, 0, true);
...
private:
    // ════════════════════════════════════════════════════════════════════════
    // INTERNAL ATOMIC STATS — Thread-safe counters for concurrent scan paths
    // ════════════════════════════════════════════════════════════════════════
    ...
    [[nodiscard]] ScanResult analyzeContent(
        ...
        if (detectAmsiBypass(normLower, amsiTechniques)) {
            ...
            notifyAmsiBypassDetected(pid, amsiTechniques);
        }
        ...
        for (const auto& [pat, score] : Patterns::SUSPICIOUS_CMDLETS) {
            if (normLower.find(pat) != std::string::npos) {
                ...
                addFlaggedLine(content, pat, flagged);
            }
        }
        ...
        // ── Phase 8: Download cradles (deduplicated from SUSPICIOUS_CMDLETS) ─
        for (const auto& [pat, score] : Patterns::DOWNLOAD_CRADLES) {
            if (normLower.find(pat) != std::string::npos) {
                totalScore += score;
                rules.push_back(std::string("Download cradle: ") + pat);
        ...
        ObfuscationDetails obf = analyzeObfuscation(content, normLower);
        ...
        if (deobfEnabled && depth < maxDepth) {
            ...
            if (!deobfuscated.empty() && deobfuscated != content) {
                auto inner = analyzeContent(deobfuscated, contextName + "_deobf", pid, depth + 1);
    ...
    [[nodiscard]] bool detectV2Downgrade(std::string_view cmdLower) const noexcept {
        ...
        for (const char* flag : V2_FLAGS) {
            ...
            if (pos != std::string_view::npos) {
                // Ensure the flag starts at a word boundary
                if (pos > 0 && !std::isspace(static_cast<unsigned char>(cmdLower[pos - 1]))) {
                    continue;
                }
    ...
    [[nodiscard]] bool detectAmsiBypass(
        ...
        // Direct pattern matches — patterns are already lowercase in the table
    ...
    [[nodiscard]] std::string deobfuscate(std::string_view content) const noexcept {
        try {
            ...
            // Try resolving [char] code concatenation:
            // [char]73+[char]69+[char]88 -> IEX
            {
                std::string resolved = resolveCharCodes(content);
            ...
            SS_LOG_ERROR(LOG_CAT, L"Exception in deobfuscation engine");
        }
    ...
    // ════════════════════════════════════════════════════════════════════════
    // CHAR CODE RESOLVER — resolves [char]N sequences
    // ════════════════════════════════════════════════════════════════════════

    [[nodiscard]] std::string resolveCharCodes(std::string_view content) const noexcept {
        try {
            std::string lower(content);
            std::transform(lower.begin(), lower.end(), lower.begin(),
                [](unsigned char c){ return static_cast<char>(std::tolower(c)); });

            constexpr std::string_view marker = "[char]";
            if (lower.find(marker) == std::string::npos) return {};

            std::string resolved;
            resolved.reserve(content.size());
            size_t pos = 0;
            bool anyResolved = false;

            while (pos < lower.size()) {
                auto found = lower.find(marker, pos);
                if (found == std::string::npos) {
                    resolved.append(content.substr(pos));
                    break;
                }
                resolved.append(content.substr(pos, found - pos));
                pos = found + marker.size();

                // Parse the integer after [char]
                int charVal = 0;
                size_t numStart = pos;
                while (pos < lower.size() && std::isdigit(static_cast<unsigned char>(lower[pos]))) {
                    charVal = charVal * 10 + (lower[pos] - '0');
                    ++pos;
                    if (charVal > 0xFFFF) break;
                }
                if (pos > numStart && charVal > 0 && charVal <= 127) {
                    resolved += static_cast<char>(charVal);
                    anyResolved = true;
                    // Skip trailing '+' concatenation operator
                    while (pos < lower.size() &&
                           (lower[pos] == '+' || std::isspace(static_cast<unsigned char>(lower[pos])))) {
                        ++pos;
                    }
                } else {
                    resolved.append("[char]");
                    resolved.append(content.substr(numStart, pos - numStart));
                }
            }

            return anyResolved ? resolved : std::string{};
        } catch (...) {
            return {};
        }
    }
    ...
    // ════════════════════════════════════════════════════════════════════════
    // BASE64 DECODE — PowerShell UTF-16LE aware, uses enterprise Base64Utils
    // ════════════════════════════════════════════════════════════════════════

    [[nodiscard]] std::string decodeBase64(std::string_view encoded) const noexcept {
        try {
            if (encoded.empty() || encoded.size() > Constants::MAX_SCRIPT_SIZE_BYTES) return {};
            ...
            if (!Utils::Base64Decode(encoded, raw, opts)) {
                return {};
            }

            if (raw.empty()) return {};

            // PowerShell -EncodedCommand uses UTF-16LE — detect and convert
            if (raw.size() >= 2 && raw.size() % 2 == 0) {
                bool looksUtf16 = true;
                size_t checkLimit = std::min<size_t>(raw.size(), 20);
                for (size_t i = 1; i < checkLimit; i += 2) {
    ...
    [[nodiscard]] ObfuscationDetails analyzeObfuscation(
        std::string_view content,
        const std::string& contentLower
    ) const noexcept {
        ...
        // Count indicators using lowered content
        for (const char* ind : Patterns::OBFUSCATION_INDICATORS) {
            size_t pos = 0;
            const size_t indLen = std::strlen(ind);
            while ((pos = contentLower.find(ind, pos)) != std::string::npos) {
                d.suspiciousTokenCount++;
                pos += indLen;
            }
        }

        // Detect types using the pre-lowered content
        if (contentLower.find("frombase64string") != std::string::npos ||
            contentLower.find("tobase64string") != std::string::npos) {
        ...
        if (contentLower.find("deflatestream") != std::string::npos ||
            contentLower.find("gzipstream") != std::string::npos) {
            if (d.primaryType == ObfuscationType::NONE)
                d.primaryType = ObfuscationType::COMPRESSION_GZIP;
            d.techniquesDetected.push_back("Compression");
        }

        // Mixed case randomization — tighter threshold to avoid flagging normal
        // camelCase/PascalCase code (typically ratio 0.15-0.30)
        if (content.size() >= 50) {
            int caps = 0, low = 0;
            for (char c : content) {
    ...
    // ════════════════════════════════════════════════════════════════════════
    // WHITELIST — Hash-based via WhitelistStore, path alone is NOT sufficient
    // ════════════════════════════════════════════════════════════════════════
    ...
    [[nodiscard]] bool isWhitelisted(
        ...
        try {
            auto* store = m_whitelistStore.load(std::memory_order_acquire);
            if (!store) return false;

            auto result = store->IsHashWhitelisted(
                sha256Hex, Whitelist::HashAlgorithm::SHA256);
            if (result.found) {
                SS_LOG_DEBUG(LOG_CAT, L"Hash whitelisted: %hs", sha256Hex.c_str());
                return true;
            }
        } catch (const std::exception& ex) {
            SS_LOG_ERROR(LOG_CAT, L"Exception in whitelist check: %hs", ex.what());
        } catch (...) {
            SS_LOG_ERROR(LOG_CAT, L"Unknown exception in whitelist check");
        }
        return false;
    }
    ...
    void notifyKernelBlock(uint32_t pid, const char* reason) const noexcept {
        ...
        try {
            ...
            AlertMsg msg{};
            msg.messageType = 0x90; // FilterMessageType_BehavioralAlert
            msg.processId   = pid;
            msg.threatScore = 100;
    ...
    [[nodiscard]] bool extractEncodedCommand(
        ...
        // Ordered longest-first so greedy match avoids false positives.
        // Each flag MUST be preceded by whitespace or be at BOL.
        ...
        for (const char* flag : FLAGS) {
            ...
            // Boundary check: flag must be at start or preceded by whitespace
            if (pos > 0 && !std::isspace(static_cast<unsigned char>(lower[pos - 1]))) {
                continue;
            }
            ...
            // Strip optional quotes around the payload
            char quoteChar = 0;
            if (start < cmdLine.size() && (cmdLine[start] == '"' || cmdLine[start] == '\'')) {
                quoteChar = cmdLine[start];
                ++start;
            }

            size_t end = start;
            if (quoteChar) {
                while (end < cmdLine.size() && cmdLine[end] != quoteChar) ++end;
            } else {
    ...
    // Searches original content case-insensitively for display-quality flagged lines.
    void addFlaggedLine(
        std::string_view content,
        const char* pattern,
        std::vector<std::pair<size_t, std::string>>& flagged
        ...
        // Build a lowered copy of original content for position-accurate search
        std::string contentLowerLocal;
        contentLowerLocal.reserve(content.size());
        for (char c : content) {
            contentLowerLocal += static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
        }

        auto pos = contentLowerLocal.find(pattern);
...
target = r'C:\ShadowStrike\ShadowStrike\src\Shared_modules\Scripts\PowerShellScanner.cpp'
